...
export class NavMenu extends Component { ... }: Defines a React class component called NavMenu, making it available for import in other files.
static displayName = NavMenu.name;: Sets a display name for debugging and React DevTools.
constructor(props) { ... }: The constructor initializes the component. It calls super(props) to pass props to the parent React.Component.
this.toggleNavbar = this.toggleNavbar.bind(this);: Binds the toggleNavbar method to the current instance, so it can access this inside the method.
this.state = { collapsed: true };: Initializes the component’s state with collapsed set to true (the navbar is collapsed by default).
toggleNavbar() { ... }: A method that toggles the collapsed state between true and false, causing the navbar to expand or collapse.


## Table of Contents

- [class based componants](#class-based-componants)
